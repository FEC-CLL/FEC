...
export default function ProductStar({ averageReview }) {
  ...
  let i = averageReview;
  for (; i >= 1; i -= 1) {
    stars.push(<span title="filled star" className="product-information__reviews--4" key={i} />);
  }
  if (i > 0 && i < 0.25) {
  ...
  for (let j = (5 - averageReview); j >= 1; j -= 1) {
    stars.push(<span title="unfilled star" className="product-information__reviews--0" key={i} />);
  }
  return stars;
}
